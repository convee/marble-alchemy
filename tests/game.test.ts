...
describe('collision rules', () => {
  it('adds repeatable power and fire to each direct hit', () => {
    ...
    run.build.power = 2;
    run.build.fire = 3;
    expect(collisionDamage(run.build, 20)).toEqual({
      direct: 6,
      chain: 0,
      total: 6,
      critical: false,
  ...
  it('critical doubles direct damage including fire, never chain damage', () => {
    const run = new Run();
    Object.assign(run.build, { power: 1, fire: 2, lightning: true, critical: true });
    expect(collisionDamage(run.build, 20, () => 0.199)).toEqual({
      direct: 8,
      chain: 2,
      total: 10,
      critical: true,
    });
    expect(collisionDamage(run.build, 20, () => 0.2).total).toBe(6);
  });
  it('lightning is capped at two other pegs and cannot recurse', () => {
    const run = new Run();
    run.build.lightning = true;
...
describe('run lifecycle', () => {
  ...
  it('does not damage enemy before settlement and rejects duplicate launches/settlements', () => {
    const run = new Run();
    const hp = run.enemyHp;
    expect(run.launch()).toBe(true);
    expect(run.launch()).toBe(false);
    run.hit(20);
    expect(run.enemyHp).toBe(hp);
    expect(run.settle()).toBe(false);
    expect(run.beginSettlement()).toBe(true);
    expect(run.beginSettlement()).toBe(false);
    expect(run.settle()).toBe(true);
    expect(run.enemyHp).toBe(hp - 1);
    expect(run.hp).toBe(4);
    expect(run.settle()).toBe(false);
    expect(run.hp).toBe(4);
    expect(run.totalDamage).toBe(1);
  });
  it('loses on zero life, and rejects further hits and launches', () => {
    ...
    expect(run.launch()).toBe(false);
    expect(run.hit(2)).toBeUndefined();
  });
  it('rolls three distinct upgrades, excluding owned unique passives', () => {
    const run = new Run();
    run.build.lightning = run.build.split = run.build.critical = true;
    for (let i = 0; i < 100; i++)
      expect(new Set(run.rollOffers())).toEqual(new Set(['power', 'fire', 'heal']));
  });
  it('repeatable upgrades stack and healing is capped at five', () => {
    const run = new Run();
    for (const id of ['power', 'power', 'fire', 'fire'] as const) {
      run.phase = 'upgrade';
      run.offers = [id];
      run.choose(id);
    }
    expect(run.build.power).toBe(2);
    expect(run.build.fire).toBe(2);
    run.phase = 'upgrade';
    run.offers = ['heal'];
    run.hp = 4;
    run.choose('heal');
  ...
  it('requires an offered upgrade and progresses through all five wins without retaliation', () => {
    ...
    for (let level = 0; level < 5; level++) {
      ...
      expect(run.hp).toBe(5);
      expect(run.choose('not-a-card' as never)).toBe(false);
      expect(run.choose(run.offers[0])).toBe(true);
