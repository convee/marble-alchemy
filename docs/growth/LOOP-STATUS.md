...
| 复玩 | GPT-6 版已形成“模型命题 → 受限规则 → 玩家五关回应 → AI 复盘 → 本地每日完成记录/分享”的闭环；匿名事件包含 `ai_challenge_loaded`、`ai_rule_triggered`、`daily_challenge_completed`、`run_complete` 和 UTM 归因，Worker 接收名单已同步 | conditional | 累积 7–14 天真实访问，再按命题来源和规则效果判断复玩率与 D1/D7 留存 |
