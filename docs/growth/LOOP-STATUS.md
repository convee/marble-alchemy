...
| 监控 | Cloudflare 最新回读为 0 page views / 0 visits（提示数据量不足）；第一方 D1 当前累计 281 events / 19 sessions / 49 page views / 8 game starts / 3 completions，最近一小时只有 1 个直接首页 session | conditional | 继续按自然/验证流量分层；等待 7–14 天真实 cohort 后再判断留存 |
...
- Cloudflare 当前 0 次 page views / 0 次 visits（数据量不足），以及第一方 D1 的 281 events / 19 sessions，都是当前观测值，不是自然获客、留存或收入证明。
