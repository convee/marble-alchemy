...
| 监控 | Cloudflare Web Analytics 已登记 `chaoschemy.com`；当前 24 小时回读 37 page views / 35 visits，来源混有验证流量 | pass-with-baseline | 继续按自然/验证流量分层；第一方 Worker 增加首页 `landing_view` 与 `cta_click` 归因 |
...
| 复盘 | 事件字典、UTM 归因、D1 导出分析器和目录分发包已提交；当前数据仍是验证流量 | conditional | 用 7–14 天真实 cohort 数据做一次继续/停止决定 |

## 不能混写的证据边界

- CI、自动对局和录屏证明产品可构建、可游玩，不证明有需求、留存或收入。
- Cloudflare 当前 37 次 page views / 35 次 visits 是混合样本，不是自然获客或收入证明。
