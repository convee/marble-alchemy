...
- 停滞约 850ms 的弹珠会被轻推；单颗弹珠存活超过 16 秒自动回收，已累计伤害保留。越界和非有限坐标也会回收。
- 速度限制为 13 个物理单位/步，降低高速穿透风险。碰撞体与可见钉子外圈一致。
- 暂停直接调用 Scene Systems 的同步暂停，冻结物理、场景时钟、粒子和结算定时器。恢复不补算暂停时间。
- 重新开始取消待结算定时器、移除所有弹珠、清空分裂队列、粒子、闪电、浮动文字、补间与配方，创建新的 Run；静态弹盘和输入监听只创建一次。
