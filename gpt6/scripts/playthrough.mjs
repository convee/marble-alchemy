...
try {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const row = {
      shot: i + 1,
      level: await page.locator('#level-number').textContent(),
      damage: await page.locator('#damage').textContent(),
      hits: await page.locator('#hits').textContent(),
      life: await page.locator('#hp-number').textContent(),
    ...
    if (await page.locator('[data-upgrade]').count()) {
      ...
      const priority =
        Number(row.life.split('/')[0]) <= 2
          ? ['heal', 'split', 'lightning', 'power', 'fire', 'critical']
          : ['split', 'lightning', 'power', 'fire', 'critical', 'heal'];
