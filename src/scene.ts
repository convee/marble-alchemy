...
export class AlchemyScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let row = 0; row < 9; row++) {
      ...
      for (let col = 0; col < count; col++) {
        ...
        this.pegs.push({
          ...
          y,
          flash: 0,
          color: [0xbca2ff, 0x8ee7cf, 0xd5bbff][(col + row) % 3],
        });
      }
    }
    ...
    this.input.on('pointerdown', (p: Phaser.Input.Pointer) => {
      if ((!p.wasTouch && p.button !== 0) || this.activePointer !== null) return;
      if (p.x < 20 || p.x > 500 || p.y < 100 || p.y > 630 || this.run.phase !== 'aiming') return;
      this.activePointer = p.id;
      this.setAimFromPoint(p.x, p.y);
      this.synth.unlock();
    });
    this.input.on('pointermove', (p: Phaser.Input.Pointer) => {
      if (p.wasTouch && this.activePointer !== p.id) return;
      if (this.run.phase === 'aiming' && p.y > 100) this.setAimFromPoint(p.x, p.y);
    });
    this.input.on('pointerup', (p: Phaser.Input.Pointer) => {
      if (p.id !== this.activePointer) return;
      // Phaser routes touchcancel through pointerup too. Cancellation is never a shot.
      if (p.event.type === 'touchcancel' || p.event.type === 'pointercancel') {
    ...
    this.input.on('pointerupoutside', (p: Phaser.Input.Pointer) => {
      if (p.id === this.activePointer) this.activePointer = null;
    });
    this.ready = true;
    this.hooks.change();
  }
  setAimFromPoint(x: number, y: number) {
    this.aim = Phaser.Math.Clamp(Math.atan2(x - 260, Math.max(35, y - 76)), -1.1, 1.1);
  }
  adjustAim(delta: number) {
  ...
  private hitPeg(ball: Ball, peg: Peg) {
    ...
    if (result.chain) {
      const nearest = this.pegs
        .filter((p) => p !== peg)
        .sort(
          (a, b) =>
            (a.x - peg.x) ** 2 + (a.y - peg.y) ** 2 - ((b.x - peg.x) ** 2 + (b.y - peg.y) ** 2),
        )
        .slice(0, 2);
      for (const target of nearest) {
        target.flash = 1;
        this.arcs.push({ from: peg, to: target, life: 220 });
        this.float(target.x, target.y - 12, '+1', '#f4d884');
      }
    }
    ...
    this.synth.tone('hit', this.run.hits);
    this.hooks.change();
  }
  private burst(x: number, y: number, color: number, count = 10) {
    for (let i = 0; i < count; i++) {
      const angle = Math.random() * Math.PI * 2;
      const speed = 0.4 + Math.random() * 2.3;
      this.particles.push({
        x,
        y,
  ...
  update(_time: number, rawDelta: number) {
    ...
    for (const [id, ball] of this.balls) {
      ...
      if (speed > 13)
        this.matter.body.setVelocity(ball.body, {
          x: (ball.body.velocity.x * 13) / speed,
          y: (ball.body.velocity.y * 13) / speed,
        });
    }
    if (
      this.run.phase === 'flying' &&
      this.balls.size === 0 &&
    ...
    this.draw();
    if (this.elapsed - this.lastUi > 150 && this.run.phase === 'flying') {
      this.lastUi = this.elapsed;
      this.hooks.change();
    }
  }
  private draw() {
    const g = this.ink.clear(),
      fx = this.effects.clear();
    // Etched brass frame, a restrained grid, and a procedural transmutation circle.
    g.lineStyle(1, 0x71628a, 0.16);
    for (let x = 32; x < WIDTH; x += 24)
      for (let y = 26; y < HEIGHT; y += 24) g.strokeCircle(x, y, 0.5);
    g.lineStyle(1, 0x9679b7, 0.12);
    g.strokeCircle(260, 355, 182);
    g.strokeCircle(260, 355, 168);
    for (let i = 0; i < 12; i++) {
      const a = (i * Math.PI) / 6;
      g.lineBetween(
        260 + Math.cos(a) * 172,
    ...
    g.lineStyle(1, 0xba94ec, 0.06);
    g.strokeTriangle(260, 178, 107, 443, 413, 443);
    g.strokeTriangle(260, 532, 107, 267, 413, 267);
    g.lineStyle(2, 0xa794c9, 0.24);
    g.strokeRoundedRect(18, 14, 484, 623, 18);
    g.lineStyle(2, 0xdcc395, 0.7);
    for (const x of [18, 502])
      for (const y of [32, 619]) {
        g.lineBetween(x, y, x, y + (y < 100 ? 20 : -20));
        g.lineBetween(x, y, x + (x < 100 ? 10 : -10), y);
      }
    // Launcher.
    const pulse = (Math.sin(this.elapsed / 450) + 1) / 2;
    g.lineStyle(1, 0xc1a5f5, 0.3);
    g.strokeCircle(260, 76, 31);
    g.strokeCircle(260, 76, 25);
    g.fillStyle(0xa786ed, 0.05 + pulse * 0.04);
    g.fillCircle(260, 76, 25);
    g.lineStyle(2, 0xd4bcff, 0.8);
    g.lineBetween(260, 76, 260 + Math.sin(this.aim) * 24, 76 + Math.cos(this.aim) * 24);
    if (this.run.phase === 'aiming') {
      // First ballistic segment only; the marker stops at the first peg/wall.
      let x = 260,
        y = 76,
        vx = Math.sin(this.aim) * 9,
        vy = Math.cos(this.aim) * 9;
      for (let i = 0; i < 42; i++) {
        x += vx;
        y += vy;
        vy += 0.22;
        if (
          x < 27 ||
          x > 493 ||
          this.pegs.some((p) => Math.hypot(p.x - x, p.y - y) < 16) ||
          y > 570
        ) {
          g.lineStyle(1, 0xddd0ff, 0.65);
          g.strokeCircle(x, y, 10);
          break;
        }
        if (i % 3 === 0) {
          g.fillStyle(0xdac8ff, 0.65 - i / 85);
          g.fillCircle(x, y, 2);
        }
      }
      g.fillStyle(0xb59aff, 0.16);
      g.fillCircle(260, 76, 17);
      g.fillStyle(0xf5eeff);
      g.fillCircle(260, 76, 6);
      g.fillStyle(0xffffff);
      g.fillCircle(258, 74, 2);
    }
    for (const peg of this.pegs) {
      g.fillStyle(peg.color, 0.04 + peg.flash * 0.15);
      g.fillCircle(peg.x, peg.y, 15 + peg.flash * 7);
      g.lineStyle(1, peg.color, 0.3 + peg.flash * 0.7);
      g.strokeCircle(peg.x, peg.y, 10);
      g.fillStyle(peg.color, 0.7 + peg.flash * 0.3);
      g.fillCircle(peg.x, peg.y, 6);
      g.fillStyle(0xffffff, 0.65);
      g.fillCircle(peg.x - 1.3, peg.y - 1.3, 1.4);
    }
    g.fillStyle(0x94e9d2, 0.025);
    g.fillRoundedRect(29, 586, 462, 40, 10);
    g.lineStyle(1, 0x8de8cf, 0.25);
    g.lineBetween(38, 608, 482, 608);
    for (let i = 0; i < 9; i++) {
      const x = 60 + i * 50;
      g.lineStyle(1, 0x9eedda, 0.2 + pulse * 0.2);
      g.lineBetween(x - 4, 616, x, 620);
      g.lineBetween(x, 620, x + 4, 616);
    }
    for (const ball of this.balls.values()) {
      const c = ball.child ? 0x8df0cd : this.run.build.fire ? 0xffbd83 : 0xcfb6ff;
      ball.trail.forEach((point, i) => {
        fx.fillStyle(c, (i / ball.trail.length) * 0.36);
        fx.fillCircle(point.x, point.y, (i / ball.trail.length) * 5);
      });
      fx.fillStyle(c, 0.13);
      fx.fillCircle(ball.body.position.x, ball.body.position.y, 14);
      fx.fillStyle(0xffffff);
      fx.fillCircle(ball.body.position.x, ball.body.position.y, 5.5);
    }
    for (const p of this.particles) {
      fx.fillStyle(p.color, p.life / p.max);
      fx.fillCircle(p.x, p.y, (p.life / p.max) * 2.5);
    }
    for (const arc of this.arcs) {
      fx.lineStyle(2, 0xf4d884, arc.life / 220);
      fx.beginPath();
      fx.moveTo(arc.from.x, arc.from.y);
      fx.lineTo((arc.from.x + arc.to.x) / 2 + 8, (arc.from.y + arc.to.y) / 2 - 6);
      fx.lineTo(arc.to.x, arc.to.y);
      fx.strokePath();
    }
  }
  pauseGame() {
    this.activePointer = null;
    if (this.ready && !this.sys.isPaused()) this.sys.pause();
  }
  resumeGame() {
  ...
  testHit(index: number) {
    const ball = this.balls.values().next().value;
    if (ball) this.hitPeg(ball, this.pegs[index]);
  }
  testRecall() {
    for (const [id, ball] of this.balls) this.removeBall(id, ball);
  }
  testRest() {
    for (const ball of this.balls.values()) {
      ball.body.ignoreGravity = true;
      this.matter.body.setPosition(ball.body, { x: 260, y: 130 });
      this.matter.body.setVelocity(ball.body, { x: 0, y: 0 });
    }
  }
  testRelease() {
    for (const ball of this.balls.values()) ball.body.ignoreGravity = false;
  }
  testStall() {
    for (const ball of this.balls.values()) {
      this.matter.body.setPosition(ball.body, { x: 260, y: 130 });
