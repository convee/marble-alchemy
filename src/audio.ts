export class Synth {
  ...
  constructor() {
    try {
      this.enabled = localStorage.getItem('alchemy-sound') !== 'off';
    } catch {
  ...
  toggle() {
    ...
    try {
      localStorage.setItem('alchemy-sound', this.enabled ? 'on' : 'off');
    } catch {
      /* Optional setting. */
    }
  ...
  tone(kind: 'hit' | 'launch' | 'win' | 'hurt' | 'upgrade', combo = 0) {
    ...
    const notes =
      kind === 'win' || kind === 'upgrade'
        ? [440, 554, 659, 880]
        : [kind === 'hurt' ? 110 : kind === 'launch' ? 320 : 580 + (combo % 12) * 45];
    notes.forEach((f, i) => {
